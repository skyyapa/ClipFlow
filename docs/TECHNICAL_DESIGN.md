# ClipFlow 0.14.0 技术说明

本文记录当前代码的真实实现。历史导入导出、内容类型筛选和队列持久化属于后续设计。
...
- 写回后记录 `GetClipboardSequenceNumber`，仅忽略对应的系统剪贴板版本，避免复制循环且不误吞用户随后复制的内容
...
- `PasteQueueEntry` 保存历史条目的不可变快照和 Original/PlainText/Table 模式
- `PasteQueueSession` 明确区分未开始、活动中和正在派发，任何时刻只允许一个 in-flight 条目
- 开始队列后第一条立即派发；剩余条目由原全局快捷键逐次推进
- 派发前等待 Ctrl/Shift/Alt 全部松开，防止呼出快捷键残留为 `Ctrl+Shift+V`
- 写入后保存剪贴板序列号，发送 `Ctrl+V` 前再次核对序列号、目标窗口及前台窗口
- 剪贴板写入、目标窗口验证或内容验证失败时调用失败完成，不移除队首
- 设置、表格预览、确认框和文件选择器显示期间屏蔽全局快捷键，避免误粘贴到对话框
- 成功发送 `Ctrl+V` 后才移除队首，并通过后台队列原子更新使用次数
- 搜索刷新与来源筛选不会影响独立保存的队列顺序

### 表格转换

- `TableTextConverter` 是无界面纯逻辑模块，支持 CRLF/LF/CR/U+2028/U+2029
- 自动检测依次尝试 Tab、逗号、中文逗号、分号和竖线；至少 75% 非空行列数一致才采用
- 连续空格只能手动选择，避免拆错姓名或地址
- 解析保留连续、末尾空字段，并支持双引号包裹与 `""` 转义；未闭合引号按原文保留
- 纯文本格式以 `\t` 分列、`\r\n` 分行；HTML 格式按 UTF-8 字节计算 CF_HTML 偏移
- HTML 单元格默认使用 `mso-number-format:'\@'`，保护前导零、长数字和公式样文字
- `TablePreviewWindow` 最多渲染 30×8 个单元格，转换结果本身不截断
...
- `GetSourceApplications` 按最近复制时间返回去重的来源进程名
- 普通查询把来源条件下推到 SQLite；失效文件视图在路径状态判断时同时过滤来源
- 应用排除名单以逗号、分号或换行解析，比较时忽略大小写和 `.exe` 后缀
- 来源排除在进入后台写入队列前执行，对文本、图片和文件列表统一生效
- 敏感文本规则只处理单行文本：识别 4–24 位数字/常见分隔形式，或 8–64 位且包含至少三类字符的高复杂度字符串
- 敏感规则默认关闭；全部判断均在本地完成，不上传内容

## 12. 当前技术债务

- FTS5 与模糊查询尚未提供可视化过滤语法
- 敏感内容识别仍是启发式规则，无法保证识别所有凭据
- 连续粘贴队列目前只保存在内存中，不跨重启恢复
...
- 收藏保护与清空未收藏记录
- 完整应用源码编译
- 队列顺序、并发保护、失败重试和完成状态的确定性检查
- 各类换行、分隔符、引号、空字段、Unicode 和公式样文字转换检查
- 含中文与 Emoji 的 CF_HTML UTF-8 字节偏移检查
- 表格预览、队列栏和“表格→队列”组合交互的 UI 自动化检查
