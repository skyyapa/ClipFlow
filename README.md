...
- 可设置不记录指定应用，并可选启用敏感单行文本保护
- 支持图片保留天数和图片空间上限
- 自动清理没有历史记录引用的孤立 PNG 文件
- 图片编码与剪贴板历史写入在后台串行处理，减少界面卡顿
- 文字卡片提供鼠标可点的纯文本粘贴按钮
- 文字卡片提供图钉收藏按钮，也可按 `Ctrl + S` 收藏
- 支持临时连续粘贴队列，按加入顺序逐条粘贴文字、图片和文件
- 多行文字可预览并转换为 TSV/HTML 表格，直接粘贴到 Excel、WPS 或 LibreOffice
- 表格模式保护前导零、长数字及公式样文字，避免表格软件自动改写
- 通过剪贴板序列号识别自身写回，不会把队列或表格结果重复记入历史
...
搜索栏右侧的应用下拉框可只查看某个来源。设置中的应用排除名单支持逗号或换行分隔，填写进程名即可，无需 `.exe`。

### 连续粘贴

1. 点击文字卡片右侧的 `+`；图片或文件可从“更多”菜单加入队列。
2. 队列栏会显示数量和下一条内容，点击“开始”会立即粘贴第一条。
3. 后续每按一次当前呼出快捷键（默认 `Ctrl + Shift + V`），就会粘贴下一条。
4. 可在队列栏清空未开始的队列，也可从托盘菜单取消进行中的队列。

队列只保存在内存中，退出 ClipFlow 后自动清空。条目失效或目标窗口不可用时，当前条目会保留，修复后可重试。

### 多行文字转表格

点击多行文字卡片右侧的 `▦`，或选中后按 `Ctrl + Enter`。预览窗口支持自动识别、每行一格、制表符、逗号、中文逗号、分号、竖线和连续空格；可直接粘贴，也可把转换结果加入连续粘贴队列。预览最多显示前 30 行 × 8 列，实际粘贴不会截断。
...
- 仅支持 Windows
- 连续粘贴队列退出程序后不会保存
- 表格转换采用本地分隔规则，不处理复杂 Excel 工作簿结构
- 敏感内容保护采用本地启发式规则，可能存在漏判，因此默认关闭
- 当前版本未进行代码签名
